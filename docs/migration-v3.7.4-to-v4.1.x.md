...
3. [Asset data model changes (base schema v0.12.0 → v1.18.0)](#3-asset-data-model-changes-base-schema-v0120--v1180)
...
## 3. Asset data model changes (base schema v0.12.0 → v1.18.0)

The base schema has been updated from `v0.12.0` to `v1.18.0`. This is a significant restructuring of the asset model.
...
| `model.ErrValidation`, `model.ErrEmpty` | Sentinel values for use with `errors.As` (see below) |
...
### Error type usage with `errors.As`

To distinguish error categories from model methods:
